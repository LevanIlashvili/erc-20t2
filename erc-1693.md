...
A compliant ERC-1693 token contract MUST maintain, in addition to the balance mapping defined in ERC-20, a data structure tracking pending (unsettled) transfers. Every transfer proceeds through one of four terminal outcomes via the following lifecycle:
...
Compliant contracts MUST return `true` for `supportsInterface(0xdc09a479)`, where `0xdc09a479` is the XOR of the function selectors defined in `IERC20T2` above.
...
A fixed on-chain period of `172800` seconds was selected over a variable business-day-aware period on the grounds of simplicity. A forthcoming extension (ERC-1693B) is anticipated to introduce holiday-calendar awareness for tokens representing assets whose underlying markets are closed on weekends.
...
[ERC-4626](./eip-4626.md) vaults that wrap an ERC-1693 asset MUST be aware that deposits and redemptions will not reflect in share balances for two days. Vault implementers are STRONGLY RECOMMENDED to maintain a parallel pending-share accounting. This is not a deficiency of this standard.

## Backwards Compatibility

This standard is intentionally not backwards compatible with ERC-20 at the semantic level. A contract presenting itself as ERC-20 but implementing ERC-1693 semantics WILL cause integration failures in consumer contracts that assume atomic settlement. Such failures are a feature of the proposal.

Integrators MUST detect ERC-1693 compliance via ERC-165 and adjust their logic accordingly. In particular:

- Decentralised exchanges MUST either reject ERC-1693 tokens or implement two-day order lifecycle management *and* recipient-acknowledgment workflows. Market-making strategies that assume silent receipt of filled orders are incompatible with this standard.
- Flash loan providers MUST reject ERC-1693 tokens as the loan asset. A flash loan over a T+2 asset that also requires recipient acknowledgment is not a flash loan.
- Liquidation engines operating on ERC-1693 collateral MUST account for the settlement window in their liquidation thresholds. Liquidators SHOULD expect that a liquidation initiated at time `t` returns collateral at time `t + 172800` at the earliest, contingent on their own acknowledgment, and SHOULD price this accordingly.
- Recipient contracts intending to receive ERC-1693 tokens MUST implement a mechanism to call `acknowledge` within the T+2 to T+7 window. Contracts that cannot call `acknowledge` (for example, because they lack privileged entry points or depend on external keeper infrastructure that may become unavailable) MUST be prepared for all inbound transfers to be reclaimed by the sender after T+7. The authors note that this requirement eliminates the majority of "send-and-forget" integration patterns currently prevalent in DeFi. This is intentional.
...
abstract contract ERC20T2 is IERC20, IERC165 {
    ...
    /// @dev Interface ID for IERC20T2, computed as the XOR of the twelve function selectors.
    bytes4 public constant IERC20T2_INTERFACE_ID = 0xdc09a479;
    ...
    // ---- Internal ----
    function _initiate(address from, address to, uint256 amount) internal {
        ...
        bytes32 tradeId = keccak256(abi.encode(from, to, amount, block.timestamp, nonce));
    ...
    /// @dev Mints `amount` tokens to `to`, crediting the available balance directly.
    /// @dev Newly minted tokens are NOT subject to the settlement delay or acknowledgment
    ///      requirement, on the rationale that there is no counterparty from whom they
    ///      could be unwound. Derived contracts SHOULD expose a permissioned wrapper.
    ...
    /// @dev Burns `amount` tokens from `from`'s available balance. In-flight tokens
    ///      cannot be burned; they must first be cancelled, acknowledged, rejected, or reclaimed.
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return
            ...
            interfaceId == IERC20T2_INTERFACE_ID;
    }
}
```
...
Between initiation and terminal state, tokens are held by the contract itself. Implementations MUST NOT permit the `_pendingTransfers` mapping to be mutated by any mechanism other than `acknowledge`, `cancel`, `reject`, or `reclaim`. Upgrade patterns that preserve this mapping are RECOMMENDED; those that do not are considered unsafe.

### Front-Running of `reclaim`

Because `reclaim` is callable by any party after expiry, a motivated observer can front-run the intended reclaimer. This is not a vulnerability; the effect of a successful `reclaim` is deterministic (funds to `from`) and identical regardless of caller. The only cost is gas, which is borne by the caller. Implementers concerned with reclamation-fee capture are directed to the (forthcoming) ERC-1693F extension.
...
Wide adoption of ERC-1693 will reduce the velocity of on-chain capital by a factor approximately equal to `1 + (total_lifecycle / average_holding_period)`. Under the updated T+7 lifecycle this factor increases relative to earlier drafts. The authors consider this a benefit.
